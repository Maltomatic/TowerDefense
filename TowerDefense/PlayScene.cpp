        ...
        for(int j = 0 ; j < std::get<2>(current) ; j++){
            ...
            switch (std::get<0>(current)) {
				case 1:
					...
					//printf("normal enemy spawned\n");
					break;
				case 2:
					EnemyGroup->AddNewObject(enemy = new SofaEnemy(SpawnCoordinate.x, SpawnCoordinate.y));
					//printf("sofa enemy spawned\n");
					break;
				case 3:
					EnemyGroup->AddNewObject(enemy = new StrongEnemy(SpawnCoordinate.x, SpawnCoordinate.y));
					//printf("strong enemy spawned\n");
					break;
                case 4:
					EnemyGroup->AddNewObject(enemy = new BuffEnemy(SpawnCoordinate.x, SpawnCoordinate.y));
					//printf("buff enemy spawned\n");
					break;
                case 5:
                    EnemyGroup->AddNewObject(enemy = new BombEnemy(SpawnCoordinate.x, SpawnCoordinate.y));
					//printf("bomb enemy spawned\n");
					break;
                case 6:
                    EnemyGroup->AddNewObject(enemy = new Vampire(SpawnCoordinate.x, SpawnCoordinate.y));
					//printf("vampire spawned\n");
					break;
...
void PlayScene::OnMouseUp(int button, int mx, int my) {
	...
	if (button & 1) {
		if (mapState[y][x] != TILE_OCCUPIED) {
		    ...
		    if(preview->CollisionRadius == 1){
                for (auto& it : scene->TowerGroup->GetObjects()) {
                    Turret* turret = dynamic_cast<Turret*>(it);
                    if (Engine::Collider::IsCircleOverlap(preview->Position, 1, turret->Position, turret->CollisionRadius)) {
                        turret->dozed();
		    ...
		    float maX = x * BlockSize + BlockSize / 2;
            float maY = y * BlockSize + BlockSize / 2;
            printf("%f %f\n", maX, maY);
		    for(auto& it: scene->TowerGroup->GetObjects()) {
                Turret* turret = dynamic_cast<Turret*>(it);
                if(turret->Position.x == maX && turret->Position.y == maY) return;
		    }
            EarnMoney(-preview->GetPrice());
			preview->GetObjectIterator()->first = false;
			UIGroup->RemoveObject(preview->GetObjectIterator());
			preview->Position.x = maX;
			preview->Position.y = maY;
			preview->Enabled = true;
			preview->Preview = false;
			preview->Tint = al_map_rgba(255, 255, 255, 255);
			TowerGroup->AddNewObject(preview);
...
void PlayScene::EarnMoney(int money) {
	this->money += money;
	UIMoney->Text = std::string("here$") + std::to_string(this->money);
	UILives->Text = std::string("Life ") + std::to_string(lives);
}
...
void PlayScene::ConstructUI() {
	...
	btn = new TurretButton("play/floor.png", "play/dirt.png",
		Engine::Sprite("play/burgerBIG.png", 540, BlockSize * (MapHeight+0.2), 0, 0, 0, 0)
		, 530, 128 * MapHeight, WallTurret::Price);
	btn->SetOnClickCallback(std::bind(&PlayScene::UIBtnClicked, this, 3));
	UIGroup->AddNewControlObject(btn);

	btn = new TurretButton("play/floor.png", "play/dirt.png",
		Engine::Sprite("play/turret-5.png", 660, BlockSize * MapHeight, 0, 0, 0, 0)
		, 650, 128 * MapHeight, UnaTurret::Price);
	btn->SetOnClickCallback(std::bind(&PlayScene::UIBtnClicked, this, 4));
    UIGroup->AddNewControlObject(btn);

    btn = new TurretButton("play/floor.png", "play/dirt.png",
		Engine::Sprite("play/dozer.png", 1130, BlockSize * (MapHeight+0.2), 0, 0, 0, 0)
		, 1130, 128 * MapHeight, Shovel::Price);
...
void PlayScene::UIBtnClicked(int id) {
    ...
    else if (id == 3 && money >= WallTurret::Price)
		preview = new WallTurret(0, 0);
    else if (id == 4 && money >= UnaTurret::Price)
		preview = new UnaTurret(0, 0);
    else if (id == 5)
